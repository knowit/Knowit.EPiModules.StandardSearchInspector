...
namespace Knowit.EPiModules.StandardSearchInspector.Business
{
    public class SearchServiceModule
    {
        ...
        public SearchServiceModule(SearchHandler searchHandler, IContentLoader contentLoader)
        {
        ...
        public virtual SearchResults Search(string searchText, IEnumerable<ContentReference> searchRoots, HttpContextBase context, string languageBranch, int hitsPerPage, int page)
        {
            var query = CreateQuery(searchText, searchRoots, context, languageBranch);
            return _searchHandler.GetSearchResults(query, page, hitsPerPage);
        }
